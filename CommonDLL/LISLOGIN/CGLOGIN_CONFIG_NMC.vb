...
Namespace CONFIG
    ...
    Public Class FN
        ...
        ' 사용자 비밀번호 확인
        Public Shared Function fnGet_UsrPwd(ByVal rsUsrID As String, ByVal rsUsrPwd As String) As Boolean
            Dim sFn As String = "Public Shared Function fnGetUsrInfo(String) As String"
            Dim sSql As String = ""
            Dim dt As New DataTable
            ...
            Try
                ...
                sSql = ""
                sSql += "SELECT usrid FROM lf090m WHERE usrid = :usrid AND usrpwd = DAMO.HASH_STR_DATA(:usrpwd)"

                alParm.Clear()
                alParm.Add(New OracleParameter("usrid", rsUsrID))
                alParm.Add(New OracleParameter("usrpwd", rsUsrPwd))
                ...
                If dt.Rows(0).Item("usrid").ToString = rsUsrID Then
                    Return True
                Else
                    Return False
                End If
        ...
        Public Shared Function fnExe_NewUsrPWD(ByVal rsUsrID As String, ByVal rsUsrPW As String) As Boolean
            ...
            Try
                ...
                sSql += "       fldval = DAMO.HASH_STR_DATA(:fldval),"
                sSql += "       regid  = :regid,"
                ...
                dbCmd.Parameters.Add("fldval", OracleDbType.Varchar2).Value = rsUsrPW
                ...
                If iRet = 0 Then
                    ...
                    sSql += "            VALUES( :usrid, :fldgbn, DAMO.HASH_STR_DATA(:fldval), :regid, fn_ack_sysdate) "

                    dbCmd.Parameters.Clear()
                    dbCmd.Parameters.Add("usrid", OracleDbType.Varchar2).Value = rsUsrID
                    dbCmd.Parameters.Add("fldgbn", OracleDbType.Varchar2).Value = "2"
                    dbCmd.Parameters.Add("fldval", OracleDbType.Varchar2).Value = rsUsrID
                    dbCmd.Parameters.Add("regid", OracleDbType.Varchar2).Value = rsUsrID
                ...
                sSql = ""
                sSql += "UPDATE lf090m SET usrpwd = DAMO.HASH_STR_DATA(:usrpwd)"
                sSql += " WHERE usrid  = :usrid"
                sSql += "   AND NVL(delflg, '0') = '0'"

                dbCmd.Parameters.Clear()
                dbCmd.Parameters.Add("usrpwd", OracleDbType.Varchar2).Value = rsUsrPW
                dbCmd.Parameters.Add("usrid", OracleDbType.Varchar2).Value = rsUsrID
