    ...
    Public Class SrhFn
        ...
        Public Function fnGet_M_AFB_Statistics(ByVal rsType As String, ByVal rsDMYGbn As String, ByVal ra_sDMY As String(), ByVal rsDT1 As String, ByVal rsDT2 As String, _
            ...
            Try
                ...
                sSql += "                                  from lf060m       "
                sSql += "                                 where testcd in ('LM20101','LM20303') "
                sSql += "  and usdt <= :dates"
        ...
        Public Function fnGet_Test_Statistics(ByVal rsType As String, ByVal rsDMYGbn As String, ByVal ra_sDMY As String(), ByVal rsDT1 As String, ByVal rsDT2 As String, ByVal rsADN As String, _
            ...
            Try
                ...
                sSql = ""
                If rsSpc = "" Then
                    sSql += "SELECT CASE WHEN 'Y' = '" + IIf(rsSame = "Y", "Y", "N").ToString + "' THEN NVL (MIN (b.samecd), b.testcd) ELSE a.testcd END testcd, '' spccd,"
                    sSql += "      b.tnmd AS tnm, '' spcnm,"
                Else
                    sSql += "SELECT CASE WHEN 'Y' = '" + IIf(rsSame = "Y", "Y", "N").ToString + "' THEN NVL (MIN (b.samecd), b.testcd) ELSE a.testcd END testcd, a.spccd spccd,"
                    sSql += "     b.tnmd AS tnm, a.spcnm,"
                End If
                sSql += "       SUM(a.stcnt" + rsAbRst + ") ctotal,"
                ...
                If rsSpc = "" Then
                    sSql += "        SELECT a.spccd spccd, a.styymmdd, a.testcd, a.stcnt, a.sthh, a.styymm, a.styy"
                Else
                    sSql += "        SELECT a.spccd spccd, c.spcnmd spcnm, a.styymmdd, a.testcd, a.stcnt, a.sthh, a.styymm, a.styy"
                End If
                ...
                '<20130910 정선영 추가, 바코드 분류 구분 적용
                If rsBcclsCd.Length > 0 Then
                    sSql += "   AND   (a.testcd, a.spccd) IN (select testcd, spccd FROM lf060m where bcclscd = :bcclscd1)"
                    alParm.Add(New OracleParameter("bcclscd1", OracleDbType.Varchar2, rsBcclsCd.Length, ParameterDirection.Input, Nothing, Nothing, Nothing, Nothing, DataRowVersion.Current, rsBcclsCd))
                End If
                '>
                ...
                sSql += "   AND a.spccd = b.spccd"
                sSql += "   AND b.usdt <= a.styymmdd || '000000'"
                sSql += "   AND b.uedt >= a.styymmdd || '235959'"
                ...
                If rsBcclsCd.Length > 0 Then
                    sSql += "           AND bcclscd = :bcclscd"
                    alParm.Add(New OracleParameter("bcclscd2", OracleDbType.Varchar2, rsBcclsCd.Length, ParameterDirection.Input, Nothing, Nothing, Nothing, Nothing, DataRowVersion.Current, rsBcclsCd))
                End If
                ...
                If rsSpc = "Y" Then
                    sSql += " GROUP BY b.samecd, b.testcd, a.testcd, b.tnmd, a.spccd, a.spcnm"
                    sSql += " ORDER BY a.testcd, a.spccd"
                Else
                    sSql += " GROUP BY b.samecd, b.testcd, a.testcd, b.tnmd"
                    sSql += " ORDER BY a.testcd"
                End If
            ...
            Try
                ...
                Else
                    ...
                    ElseIf rsDMYGbn = "M" Then
                        ...
                        sSql += "                  ) a1 ON a.slipcd = a1.slipcd AND a.days = a1.days"
                    Else
                        sSql += "       LEFT OUTER JOIN (SELECT f.partcd || f.slipcd, t.styy days, SUM(t.stcnt) stcnt FROM lt020m t, lf060m f"
                        sSql += "                    WHERE t.styy  >= :dates AND t.styy <= :datee"
                        sSql += "                      AND t.sttype = :sttype"
                        sSql += "                      AND t.testcd = f.testcd"
                        sSql += "                      AND t.spccd  = f.spccd"
