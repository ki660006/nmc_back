...
Public Class DA_ST_CYTOSPIN
    ...
    Public Shared Function fnGet_nCov(ByVal rsTestCd As String, ByVal rsClscd As String, ByVal r_dbCn As OracleConnection) As DataTable
        ...
        Try
            ...
            sSql = ""
            sSql += " SELECT CLSVAL                       " + vbCrLf
            sSql += "   FROM LF000M                       " + vbCrLf
            sSql += "  WHERE CLSGBN = 'NCOV'              " + vbCrLf
            sSql += "    AND CLSCD LIKE :clscd || '%'     " + vbCrLf 'C = 코멘트 I = 개요 S = 검체
            sSql += "    AND CLSDESC LIKE :testcd || '%'  " + vbCrLf
