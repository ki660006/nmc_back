...
Public Class FGCDHELP_TEST_NEW
    ...
    Private Sub sbDisplay_TestSpc(ByVal rsTestCd As String, ByVal rsSpcCd As String)
        ...
        Try
            ...
            If dt.Rows.Count > 0 Then
                ...
                If dt.Rows(0).Item("enforcement").ToString = "0" Then
                    CheckBox3.Checked = True
                ElseIf dt.Rows(0).Item("enforcement").ToString = "1" Then
                    CheckBox2.Checked = True
                ElseIf dt.Rows(0).Item("enforcement").ToString = "2" Then
                    CheckBox1.Checked = True
                ElseIf dt.Rows(0).Item("enforcement").ToString = "3" Then
                    CheckBox4.Checked = True
                End If

                If dt.Rows(0).Item("request").ToString = "0" Then
                    CheckBox7.Checked = True
                ElseIf dt.Rows(0).Item("request").ToString = "1" Then
                    CheckBox6.Checked = True
                ElseIf dt.Rows(0).Item("request").ToString = "2" Then
            ...
            If dt.Rows.Count > 0 Then
                With spdTestInfo
                    ...
                    For ix As Integer = 0 To dt.Rows.Count - 1
                        dt_Ref = (New CDHELP.DA_CDHELP_TEST).fnGet_test_ref(dt.Rows(ix).Item("testcd").ToString.Trim, rsSpcCd, Me.txtUsDt.Text.Replace("-", "").Replace(":", "").Replace(" ", ""))
        ...
        Try
            ...
            With spdTestInfo
                ...
                If Me.lblTest.Text.Trim = "처방코드" Then
                    ...
                    sTordcd = .Text
                    If Me.txtTCode.Text <> sTordcd Then
                ...
                ElseIf Me.lblTest.Text.Trim = "검사코드" Then
                    ...
                    sTestcd = .Text
                    If Me.txtTCode.Text <> sTestcd Then
                        COMMON.CommXML.setOneElementXML(msXML, msTestFile, "TEST", Me.txtTCode.Text + "^" + Me.lblTest.Text.Trim)
                        ' Me.btnUp.Visible = True
    ...
    Public Sub sbPrint(Optional ByVal ExcelYN As Boolean = False)
        ...
        Dim xlsWkSs As Excel.Worksheet = Nothing


        Try
            xlsApp = CType(GetObject("", "Excel.Application"), Excel.Application)
            xlsWkB = xlsApp.Workbooks.Open(Windows.Forms.Application.StartupPath + "\SSF\test.xls")
            ...
            '-- 시행처   [√]원내 [  ]원외 [  ]국가기관 보건환경연구원 [  ]국가기관 질병관리본부
            ...
            If CheckBox5.Checked = "1" Then ' 해당없음
                xlsWkS.Range("B" + (3 + 13 * (iCnt - 1)).ToString).Value = "[√]해당없음"
            Else
            ...
            xlsWkS.Range("D" + (6 + 13 * (iCnt - 1)).ToString).Value = "단위"
            ...
            '-- 8
            '<JJH 이미지 엑셀로 넣어주는 부분
            Dim file As String = Windows.Forms.Application.StartupPath + "\tube.png"
            ...
            With spdTestInfo
                ...
                Else
                    For i As Integer = 1 To Spdrow
                        Set_WksBorder(xlsWkS, CStr(11 + i)) '세부검사 수만큼 셀 추가,병합
            ...
            If ExcelYN Then 'To Excel
                Dim sDir As String = Windows.Forms.Application.StartupPath + "\Excel"
                If IO.Directory.Exists(sDir) = False Then IO.Directory.CreateDirectory(sDir)

                xlsWkSs = xlsWkS
                Dim Dir As String = Windows.Forms.Application.StartupPath + "\Excel\" + Me.txtTnmd.Text + ".xls"

                'xlsWkS.SaveAs(Windows.Forms.Application.StartupPath + "\Excel\excel.xls")
                xlsWkSs.SaveAs(Dir) '엑셀저장 


                Process.Start(Dir)  '엑셀실행
            ...
            Else '출력
                ...
                'xlsWkS.ExportAsFixedFormat(Excel.XlFixedFormatType.xlTypePDF)
        ...
        Catch ex As Exception
            xlsWkS = Nothing
            xlsWkSs = Nothing
            xlsWkB.Close(False) : xlsWkB = Nothing
    ...
    Public Sub Set_WksBorder(ByVal xlsWkS As Excel.Worksheet, ByVal Range As String)

        Try

            With xlsWkS
                '-- 셀 추가
                xlsWkS.Range("A" + Range).Rows.Insert()
                xlsWkS.Range("A" + Range).RowHeight = 33

                '--세부검사목록 병합
                xlsWkS.Range("A11", "A" + Range).Merge()
                If Range = "12" Then '-- 첫줄 추가시 병합 해제 진행
                    xlsWkS.Range("B13").MergeArea.UnMerge()
                    xlsWkS.Range("B13:I13").UnMerge()
                End If

                '--병합
                xlsWkS.Range("B" + Range + ":E" + Range).Merge()
                xlsWkS.Range("F" + Range + ":H" + Range).Merge()
...
Public Class DA_CDHELP_TEST_NEW
    ...
    Public Function fnGet_testspc_info(ByVal rsTestCd As String, ByVal rsSpcCd As String) As DataTable
        ...
        Try
            ...
            sSql += "       f6.tatyn, f6.prptmi, f6.frptmi, f6.erptmi"
    ...
    Public Function fnGet_testspc_ref(ByVal rsTestCd As String, ByVal rsSpcCd As String, ByVal rsUsDt As String) As DataTable
        ...
        Try
            ...
            sSql += "SELECT DISTINCT"
            sSql += "       f6.tcdgbn, f6.testcd, f6.tordcd, f6.sugacd, f6.edicd, f6.tnmd, 1 sort1, f6.dispseqo sort2"
            sSql += "  FROM lf060m f6"
            sSql += " WHERE f6.testcd  LIKE :testcd || '%'"
            sSql += "   AND f6.spccd   = :spccd"
            sSql += "   AND f6.usdt    = :usdt"
            sSql += " UNION "
            sSql += "SELECT f6.tcdgbn, f6.testcd, f6.tordcd, f6.sugacd, f6.edicd, f6.tnmd, 2 sort2, f6.dispseqo sort2"
            sSql += "  FROM lf062m f62, lf060m f6"
