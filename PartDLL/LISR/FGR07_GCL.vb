...
Public Class FGR07_GCL
        ...
        For ix As Integer = 0 To m_al_StSub.Count - 1
            ...
            If ix < r_al_FileNm.Count Then
                ...
                If CType(m_al_StSub(ix), StSubInfo).Type = "2" Then
                    ...
                    Me.picBuf.Dispose()
                    Me.picBuf.Image = Nothing
                    bmpBuf.Dispose() 'JJH 재조회시 프로세서가 해당 이미지를 잡고있는 문제로 dispose
                            ...
                            If iReturn > 0 Then
                                ...
                                If rsRstFlg = "3" Then
                                    ...
                                    If (New LISAPP.APP_R.AxRstFn).fnReg_IMAGE(sBcNo, sTestcd, alFileNm) Then
                                        Dim bRet As Boolean = fnSend_GCLImg_Info(sBcNo, sTestcd)
    ...
    Private Function fnSend_GCLImg_Info(ByVal rsBcno As String, ByVal rsTestcd As String) As Boolean
        ...
        oraDbCn = DBORA.DbProvider.GetDbConnection_GCRL
        ...
        Try
            ...
            sSql = ""
            sSql += "UPDATE GCRL.UPLOADMST       " '21년 12 사용하고 있는 서버 
            'sSql += "UPDATE GC_NLIS.UPLOADMST       " '22년 jhs 녹십자 최신서버 변경 ' GCLTEST
            sSql += "   SET HOSPI_DOWN_YN = 'Y'  "
            sSql += " WHERE CSTCD     = '41666'  "
            sSql += "   AND SAMPLENO  = :BCNO    "
            sSql += "   AND CSTITEMCD = :TESTCD  "
            ...
            With oraDbCmd
                ...
                .Parameters.Clear()
                .Parameters.Add("KSEQ", OracleDbType.Varchar2).Value = rsBcno
                .Parameters.Add("HGCD", OracleDbType.Varchar2).Value = rsTestcd
                    ...
                    With spdResult
                        ...
                        If strBcNo = aryBuf(0) And strtestcd = aryBuf(1) Then
                            If riCnt > 0 Then
                                ...
                                If rRegStep = 3 Then
                                    Dim bRet As Boolean = fnSend_GCLImg_Info(strBcNo, strtestcd) '삼광 서버 완료 상태 UPDATE
                                End If
...
#Region " Windows Form 디자이너에서 생성한 코드 "
    ...
    <System.Diagnostics.DebuggerStepThrough()> Private Sub InitializeComponent()
        ...
        Dim resources As System.ComponentModel.ComponentResourceManager = New System.ComponentModel.ComponentResourceManager(GetType(FGR07_GCL))
        ...
        Me.btnImage.FocalPoints.FocusPtX = 0!
        Me.btnImage.FocalPoints.FocusPtY = 0!
        ...
        Me.btnFN.FocalPoints.CenterPtY = 0!
...
#End Region
    ...
    Private Function fnGet_GCL_Img(ByVal RsDateGbn As String, ByVal RsDateS As String, ByVal RsDateE As String) As Integer

        Dim sFn As String = "Public Shared Function fnGet_GCL_Img() As String"
        ...
        oraDbCn = DBORA.DbProvider.GetDbConnection_GCRL()
        'oraDbCn = DBORA.DbProvider.GetDbConnection()
        oraDbTrans = oraDbCn.BeginTransaction()
        ...
        Try
            ...
            sSql += "   FROM GCRL.UPLOADMST A, GCRL.VIEWIMG_NMC B " ' 21년 12월 실제 운영 서버 
            'sSql += "   FROM GC_NLIS.UPLOADMST A, GC_NLIS.VIEWIMG_NMC B " ' 22년 사용할 운영 서버  ' GCLTEST
            ...
            '톄쓰트
            'sSql = ""
            'sSql += " SELECT B.TESTCD, A.SPCCD, '20190831160923' AS REQDTE, B.FILEBIN AS IMGFILE, A.REGNO, A.PATNM, A.BCNO "
            'sSql += "   FROM LJ010M A, LRS13M B"
            'sSql += "  WHERE A.BCNO = '20190831E100090' "
            'sSql += "    AND A.BCNO = B.BCNO "
            'sSql += "    AND B.TESTCD = 'LE201' "
            ...
            End If
            sSql += "   ORDER BY A.REQDTE , A.SAMPLENO , A.CSTITEMCD "
            ...
            With oraDbCmd
                ...
                Dim objDAdapter As New OracleDataAdapter(oraDbCmd)
                objDAdapter.Fill(dt)

                Dim sDir As String = "C:\수탁검사\녹십자\image\" ' Application.StartupPath + "\Image\"
                ...
                If dt.Rows.Count > 0 Then
                    ...
                    For ix As Integer = 0 To dt.Rows.Count - 1
                        Dim HGCD As String = dt.Rows(ix).Item("TESTCD").ToString
                        Dim KCCD As String = dt.Rows(ix).Item("SPCCD").ToString
                        Dim JSDT As String = dt.Rows(ix).Item("REQDTE").ToString
                        Dim SEQ As String = dt.Rows(ix).Item("seqno").ToString
                        ...
                        Dim obj As Object = dt.Rows(ix).Item("IMGFILE")  '바이너리 이미지

                        Dim a_btReturn() As Byte = CType(obj, Byte())

                        Dim filenm = dt.Rows(ix).Item("regno").ToString + "_" + dt.Rows(ix).Item("patnm").ToString + "_" + dt.Rows(ix).Item("bcno").ToString + "_" + SEQ + ".jpg"

                        Dim sFileNm = sDir + filenm '+ dt.Rows(ix).Item("FILENM").ToString '파일명
                        Dim RsTestcd = dt.Rows(ix).Item("TESTCD").ToString '검사코드
    ...
    Private Sub sbOpenImg_GCL()
        ...
        Dim iret As Integer = fnGet_GCL_Img(sDateGbn, sSdate, sEdate)
...
CallingConvention:=CallingConvention.StdCall)> _
    ...
    Private Sub sbOpenFile_GCL()
        ...
        Dim JSDT As String, KSEQ As String, HGCD As String, HGNM As String, KCCD As String, KCNM As String,
            ...
            IDAT As String, ITIM As String, ISOK As String, UDAT As String, UTIM As String, LONGRESULT As String
        Try
            Dim dt As DataTable = New DataTable
            Dim Arr_Image As New ArrayList
            Dim alList As New ArrayList

            dt = LISAPP.APP_EXLAB.fnExe_Get_Data_GCL(sDateGbn, sSdate, sEdate)
            ...
            Else
                With spdResult
                    For i As Integer = 0 To dt.Rows.Count - 1
                        ...
                        CHRT = dt.Rows(i).Item("RST").ToString ': RIMG = dt.Rows(i).Item("RIMG").ToString
                        'RMKK = dt.Rows(i).Item("RMKK").ToString ': LOHI = dt.Rows(i).Item("LOHI").ToString : CHAM = dt.Rows(i).Item("CHAM").ToString
                        IDAT = dt.Rows(i).Item("REQDTE").ToString ': ITIM = dt.Rows(i).Item("ITIM").ToString : ISOK = dt.Rows(i).Item("ISOK").ToString
                        ...
                        CHNO = "L9999999" 'dt_regno.Rows(0).Item("PID2").ToString
