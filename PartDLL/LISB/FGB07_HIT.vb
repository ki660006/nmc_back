...
Public Class FGB07
    ...
    Private Sub sb_DisPlaySubData(ByVal riRow As Integer)
        Try
            ...
            ' 혈액은행 보유혈액 조회
            dt = CGDA_BT.fn_GetStoreBldList(sTnsnum, sAbo, sRh)
                ...
                For i As Integer = 0 To r_dt.Rows.Count - 1
                    ...
                    If sState = "3"c Then
                        ...
                        For j As Integer = 1 To 4
                            If j <= iCrosslevel Then
                                .Col = .GetColFromID("rst" + j.ToString) : .Text = r_dt.Rows(i).Item("rst" + j.ToString).ToString
                                .CellType = FPSpreadADO.CellTypeConstants.CellTypeStaticText
                            Else
                    ...
                    Else
                        For j As Integer = 1 To 4
                            ...
                            Else
                                '.Col = .GetColFromID("rst" + j.ToString) : .Text = r_dt.Rows(i).Item("rst" + j.ToString).ToString : If .Text = "" Then .Text = "" <가출고 결과가 차수가 아니면 결과 안보이게
                                .Col = .GetColFromID("rst" + j.ToString) : .Text = "" : If .Text = "" Then .Text = ""
                                .CellType = FPSpreadADO.CellTypeConstants.CellTypeStaticText
                            End If
    ...
    Private Sub sb_SubSearchCB()
        ...
        ' 혈액은행 보유혈액 조회
        dt = CGDA_BT.fn_GetStoreBldList(ls_TnsNum, ls_Abo, ls_Rh)
        sb_DisPlayBldList(dt)
    End Sub
            ...
            With spdPreList
                ...
                For i As Integer = 1 To .MaxRows
                    ...
                    If sState = "1"c Or sState = "2"c Then
                        ...
                        .Col = .GetColFromID("comcd") : lcls_cross.COMCD = .Text
                        .Col = .GetColFromID("comnm") : lcls_cross.COMNM = .Text
    ...
    Private Sub txtBcOrder_KeyDown(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyEventArgs) Handles txtBcOrder.KeyDown
        ...
        Try
            ...
            Dim spcflg = dt.Rows(0).Item("spcflg").ToString

            '< 2019-02-11 JJH 접수된 검체만 되도록
            If spcflg = "2" Then
                CDHELP.FGCDHELPFN.fn_PopMsg(Me, "I"c, "접수되지 않은 검체 입니다.")
                txtOrderNum.Focus()
                txtOrderNum.SelectAll()
                Return
            End If
    ...
    Private Sub chkAbo_CheckedChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles chkAbo.CheckedChanged
        ...
        Try
            ...
            ' 혈액은행 보유혈액 조회
            dt = CGDA_BT.fn_GetStoreBldList(sTnsNo, sAbo, sRh)
            sb_DisPlayBldList(dt)
