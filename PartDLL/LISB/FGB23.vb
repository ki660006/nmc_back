...
Public Class FGB23
        ...
        Try
            ...
            Select Case tbcMain.SelectedIndex
                ...
                Case 1
                    ...
                    ' <-- 2019-03-27 JJH 월별조회시 구간 지정해서 조회되도록
                    'If dtpDate0.Value > dtpDate1.Value Then
                    ...
                    'If dateiff > 11 Then
                    '    MsgBox("월별통계 구간은 11개월까지 조회 할 수 있습니다." + vbCrLf + "날짜구간을 다시 설정하십시오!!")
                    '    Return
                    'End If
    ...
    Private Sub sb_SetHeader(ByVal rsGbn As String, ByVal rsDate As String)
        ...
        ElseIf rsGbn = "Y"c Then
            With spdYearList
                ...
                '.Col = 5 : .Text = rsDate
            End With
            'ElseIf rsGbn = "M"c Then    '' 2019-02-08 JJH 월별조회
            '    With spdMonthList
            '        '.Row = FPSpreadADO.CoordConstants.SpreadHeader + 0
            '        '.Col = 5 : .Text = rsDate + "월"
            '        '.Col = 2 : .Text = (CInt(rsDate) - 1).ToString
    ...
    Private Sub sb_DisplayDataList(ByVal r_dt As DataTable, Optional ByVal a_date As String() = Nothing)
            ...
            Select Case Me.tbcMain.SelectedIndex
                Case 0
                    With Me.spdDayList
                        ...
                        For i As Integer = 0 To r_dt.Rows.Count - 1
                            .MaxRows += 1
                            .Row = .MaxRows
                            Dim asdf As Integer = r_dt.Columns.Count
                ...
                Case 1
                    ...
                    With Me.spdMonthList
                        ...
                        ' 2019-03-27 JJH 컬럼 hidden 설정 (지정한 구간만 보이기 위해)
                        For i As Integer = 5 To 40
                            .Col = i
                            .ColHidden = True
                        Next
                        ...
                        For i As Integer = 0 To r_dt.Rows.Count - 1
                            ...
                            '''' < 2019-02-08 JJH 월별조회
                            '.Col = .GetColFromID("am1") : .Text = r_dt.Rows(i).Item("am1").ToString
                            '.Col = .GetColFromID("pm1") : .Text = r_dt.Rows(i).Item("pm1").ToString
                            '.Col = .GetColFromID("per1") : .Text = r_dt.Rows(i).Item("per1").ToString
                            ''''' >

                            'For k As Integer = 1 To 12
                            For k As Integer = 1 To CInt((r_dt.Columns.Count - 6) / 3) ' 2019-03-27 JJH 지정한 구간 spread 설정

                                .Row = .MaxRows

                                .Col = .GetColFromID("am" + k.ToString) : .Text = r_dt.Rows(i).Item("am" + k.ToString).ToString : .ColHidden = False
                                .Col = .GetColFromID("pm" + k.ToString) : .Text = r_dt.Rows(i).Item("pm" + k.ToString).ToString : .ColHidden = False
                                .Col = .GetColFromID("per" + k.ToString) : .Text = r_dt.Rows(i).Item("per" + k.ToString).ToString : .ColHidden = False

                                '.Col = .GetColFromID("am1") : .Text = r_dt.Rows(i).Item("am").ToString
                                '.Col = .GetColFromID("m1") : .Text = r_dt.Rows(i).Item("m").ToString
                                '.Col = .GetColFromID("pm1") : .Text = r_dt.Rows(i).Item("pm").ToString

                                .CellType = FPSpreadADO.CellTypeConstants.CellTypeStaticText
                                .TypeHAlign = FPSpreadADO.TypeHAlignConstants.TypeHAlignRight
                                .TypeVAlign = FPSpreadADO.TypeVAlignConstants.TypeVAlignCenter

                                ' 2019-03-27 JJH 컬럼헤드명 추가
                                For ix As Integer = 0 To a_date.Length - 1
                                    .Row = 0
                                    .Col = .GetColFromID("am" + k.ToString)

                                    If dateadd.Count = 0 Then
                                        .Text = a_date(ix).ToString + "월"
                                        dateadd.Add(a_date(ix).ToString)
                                        Exit For
                                    Else
                                        If dateadd.Contains(a_date(ix)) = True Then
                                            Continue For
                                        Else
                                            .Text = a_date(ix).ToString + "월"
                                            dateadd.Add(a_date(ix).ToString)
